...
@section('title', 'Paid Singles — ' . $season->name)
@section('page-title', 'Paid Singles')
...
<style>
    ...
    .ur-badge--paid    { background:#d1fae5;color:#059669; }
    .ur-badge--singles { background:#dbeafe;color:#2563eb; }
    .ur-amount { font-family:'DM Mono',monospace;font-weight:700;color:#059669;font-size:.85rem; }
    .ur-amount--na { color:#94a3b8; }
    ...
    .ur-btn-delete {
        display:inline-flex;align-items:center;gap:4px;
        padding:5px 11px;border-radius:7px;font-size:.72rem;font-weight:700;
        background:#fee2e2;color:#dc2626;text-decoration:none;white-space:nowrap;
        border:none;cursor:pointer;transition:background .15s;
    }
    .ur-btn-delete:hover { background:#fecaca;color:#b91c1c; }
...
<div class="ur-header">
    <div>
        <h2>Paid Singles</h2>
        <p>{{ $season->name }} &mdash; all individually paid registrations.</p>
    </div>
...
<div class="ur-breadcrumb">
    ...
    <i class="fas fa-chevron-right" style="font-size:.6rem;"></i>
    <a href="{{ route('admin-users.paid.categories', $season->id) }}">{{ $season->name }}</a>
    <i class="fas fa-chevron-right" style="font-size:.6rem;"></i>
    <span>Singles</span>
</div>
...
<div class="ur-card">
    <div class="ur-toolbar">
        <div class="ur-toolbar-left">
            <h6>Paid Singles &nbsp;<span class="ur-count-chip"><i class="fas fa-user"></i> {{ number_format($count) }} players</span></h6>
            <small>Only <strong>paid</strong> registrations shown</small>
        </div>
        <input type="text" class="ur-search" id="urSearch"
               placeholder="&#xf002;  Search player…" oninput="filterTable()">
    </div>

    <div style="overflow-x:auto;">
        @if($players->isEmpty())
            <div class="ur-empty">
                <i class="fas fa-user-check"></i>No paid singles registrations found.
            </div>
        @else
            <table class="ur-table" id="urTable">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="font-weight: 600; color: #64748b; font-family: 'DM Mono', monospace;">
                        ...
                        <td style="font-size:.72rem;font-family:'DM Mono',monospace;color:#f59e0b;font-weight:700;">
                            {{ $p->season_id ?? '—' }}
                        </td>
                            ...
                            @if($p->profile_photo)
                                <a href="{{ asset('storage/' . $p->profile_photo) }}" target="_blank">
                                    <img src="{{ asset('storage/' . $p->profile_photo) }}"
                                         style="width:80px;height:auto;border-radius:6px;object-fit:cover;border:2px solid #e2e8f0;">
                                </a>
                            @else
                            ...
                            @if($p->aadhar_proof)
                                <a href="{{ asset('storage/' . $p->aadhar_proof) }}" target="_blank">
                                    <img src="{{ asset('storage/' . $p->aadhar_proof) }}"
                                         style="width:80px;height:auto;border-radius:4px;object-fit:cover;border:2px solid #e2e8f0;">
                                </a>
                            @else
                                <span style="color:#94a3b8;">—</span>
                            @endif
                        ...
                        <td>
                            <span class="ur-badge ur-badge--singles">
                                <i class="fas fa-user" style="font-size:.5rem;"></i> Singles
                            </span>
                        </td>
