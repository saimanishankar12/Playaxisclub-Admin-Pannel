...
@section('title', 'Paid Doubles — Select Age — ' . $season->name)
...
<div class="ur-header">
    <div>
        <h2>Paid Doubles — Select Age Category</h2>
        <p>{{ $season->name }}</p>
    </div>
...
<div class="ur-breadcrumb">
    ...
    <i class="fas fa-chevron-right" style="font-size:.6rem;"></i>
    <span>Doubles — Age</span>
</div>
<div class="ur-age-intro">
    <h5>Which age category?</h5>
    <p>Select an age group to view the paid doubles registrations.</p>
</div>
@php
...
<div class="ur-age-grid">
    ...
    <a href="{{ $cnt > 0 ? route('admin-users.paid.doubles', [$season->id, urlencode($age)]) : '#' }}"
       class="ur-age-card ur-age-card--{{ $cls }} {{ $cnt === 0 ? 'ur-age-card--zero' : '' }}">
        ...
        <div class="ur-age-card-body">
            ...
            <span class="ur-age-count"><i class="fas fa-users" style="font-size:.65rem;"></i> {{ number_format($cnt) }} player{{ $cnt !== 1 ? 's' : '' }}</span>
        </div>
