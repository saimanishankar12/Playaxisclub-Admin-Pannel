...
@section('title', 'Paid Doubles — ' . $season->name)
@section('page-title', 'Paid Doubles')
...
<div class="ur-header">
    <div>
        <h2>Paid Doubles</h2>
        <p>{{ $season->name }} &mdash; all paid pair registrations.</p>
    </div>
...
<div class="ur-card">
    <div class="ur-toolbar">
        <div class="ur-toolbar-left">
            <h6>Paid Doubles &nbsp;<span class="ur-count-chip"><i class="fas fa-users"></i> {{ number_format($count) }} pairs</span></h6>
            <small>Each row = one paid doubles pair</small>
        </div>
        <input type="text" class="ur-search" id="urSearch" placeholder="&#xf002;  Search player…" oninput="filterTable()">
    </div>
    <div style="overflow-x:auto;">
        @if($pairs->isEmpty())
            <div class="ur-empty"><i class="fas fa-users"></i>No paid doubles registrations found.</div>
        @else
            <table class="ur-table" id="urTable">
                ...
                <tbody>
                    ...
                    @php $p1 = $pair['player1']; $p2 = $pair['player2']; $pay = $pair['payment'] ?? null; @endphp
                    <tr>
                        ...
                        {{-- Payment ID --}}
                        <!-- <td style="font-size:.72rem;font-family:'DM Mono',monospace;color:#64748b;">{{ $pay->razorpay_payment_id ?? '—' }}</td> -->
